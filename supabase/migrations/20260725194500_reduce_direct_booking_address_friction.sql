...
begin
  ...
  v_unique_count := coalesce(cardinality(array(select distinct unnest(p_test_ids))), 0);
  ...
  select
    ...
    count(*) filter (where mt.lab_visit_required = false and mt.home_collection_available = false)::integer,
    coalesce(array_agg(coalesce(nullif(trim(mt.common_name), ''), mt.name_sheet) order by coalesce(nullif(trim(mt.common_name), ''), mt.name_sheet)), '{}'::text[]),
  ...
  where mt.id = any(p_test_ids) and mt.is_active = true;
  ...
  if not v_has_lab_visit then
    if p_collection_address_id is null then
      ...
      select ca.* into v_address
      from public.collection_addresses ca
      where ca.id = p_collection_address_id and ca.user_id = v_user_id;
    end if;
    ...
    where id = v_address.id and user_id = v_user_id;
  end if;

  select u.* into v_patient
  from public.users u
  ...
  insert into public.orders (
    ...
    v_address.longitude,
    case
      when v_address.location_type in ('approximate', 'precise') then v_address.location_type
      when v_address.id is not null then 'precise'
      else null
    end,
    array[jsonb_build_object('status', 'booking_requested', 'timestamp', timezone('utc', now()), 'source', 'direct_test')]::jsonb[]
